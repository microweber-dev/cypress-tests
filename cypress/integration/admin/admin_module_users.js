...
describe('The Admin Page 2', function() {
    ...
    it('successfully loads', function() {
		...
		cy.wait(1500).get('#user_edit_admin_panel').find('input[name=username]').type(`new_user`)
        cy.wait(1500).get('#user_edit_admin_panel').contains('Change Password').click();
		cy.wait(1500).get('#user_edit_admin_panel').find('input[name=password]').type(`new_pass`)
		cy.wait(1500).get('#user_edit_admin_panel').find('input[name=email]').type(`johnDoe@john.doe`)
		cy.wait(1500).get('#user_edit_admin_panel').find('input[name=first_name]').type(`user fname`)
		cy.wait(1500).get('#user_edit_admin_panel').find('input[name=last_name]').type(`user pass`)
